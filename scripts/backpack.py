...
class Backpack:
    ...
    def render(self, surf, scale, get_item=False):
        ...
        surf.blit(
            backpack_text, (background.centerx - backpack_text.get_width() // 2, curr_y)
        )
        left = background.x + self.border_offset
        curr_y += backpack_text.get_height() + 4

        backpack = pygame.Rect(
            left, curr_y, self.spot_size * COLUMNS, self.spot_size * ROWS
        )
        ...
        for row in range(ROWS):
            for col in range(COLUMNS):
                ...
                if i < len(curr_items):
                    if self.render_item(
                        surf, curr_items[i], curr_spot, col, row, scale
                    ):  # myszka jest nad obrazkiem
                        item = self.items[curr_items[i]][1]
                        item_text = text_image(f"{item.name}: {item.desc}", 10)
                        text_rect = pygame.Rect(
                            0, 0, item_text.get_width(), item_text.get_height()
                        )
    ...
    def render_item(self, surf, item, curr_spot, col, row, scale):
        ...
        surf.blit(
            pygame.transform.scale(item_img, (self.item_size, self.item_size)),
