    ...
    def render_game(self, scale):
        ...
        if self.curr_state == MiniGameStates.START:
            ...
            if not self.player_squares and self.cooldown >= time():  # runda trwa
                self.draw_squares(
                    scale, [self.squares[self.round_num]], (3, 102, 252), True
                )
                ...
                if self.check_cooldown < time():  # pora na rundę (koniec sprawdzania)
                    if (
                        self.player_squares
                        and self.squares[self.round_num] == self.player_squares[0]
                    ):
                        self.won_rounds += 1
                    self.do_we_check = False
                    self.player_squares = []
            ...
            won_rounds_txt = text_image(
                f"{self.won_rounds}/{self.rounds}", 90, "data/fonts/Retro.ttf"
            )
            self.surf.blit(
                won_rounds_txt,
                (self.surf.get_width() // 12, self.surf.get_height() - 150),
            )
