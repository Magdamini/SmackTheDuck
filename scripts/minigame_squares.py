...
class MinigameSquares(MiniGame):
    def __init__(self, surf):
        ...
        self.buttons[MiniGameStates.PLAY] = Button(
            self.background.right - SUBMIT_BUTTON_SIZE[0] - self.button_offset,
            self.background.bottom - SUBMIT_BUTTON_SIZE[1] - self.button_offset,
            pygame.transform.scale(
                load_image("buttons/submit.png"),
                (SUBMIT_BUTTON_SIZE[0], SUBMIT_BUTTON_SIZE[1]),
            ),
        )
    ...
    def draw_squares(self, scale, sqrs, color=(255, 255, 255), click=False):
        sq_background = pygame.Rect(
            0, 0, self.square_size * self.cols, self.square_size * self.rows
        )
        sq_background.centerx = (
            self.background.left + (self.width - self.right_offset) // 2
        )
        ...
        for row in range(self.rows):
            for col in range(self.cols):
                ...
                pygame.draw.rect(
                    self.surf, (0, 0, 0), curr_spot, self.border_width // 2
                )
