...
class Player:
    ...
    def update(self, tilemap, movement=(0, 0)):
        ...
        is_running = 1
        if self.running:
            is_running = 2  # setting running speed
        ...
        if movement[0] > 0:
            new_action = PlayerActions.RIGHT
        elif movement[0] < 0:
            new_action = PlayerActions.LEFT

        if movement[1] > 0:
            new_action = PlayerActions.DOWN
        elif movement[1] < 0:
            new_action = PlayerActions.UP
    ...
    def render(self, surf, offset=(0, 0)):
        surf.blit(
            self.animation.get_img(), (self.pos[0] - offset[0], self.pos[1] - offset[1])
        )
