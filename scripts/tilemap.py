...
PHYSICS_TILES = {
    "Cave": {1, 2, 3, 9, 10, 11, 12, 13, 14, 15},
...
class Tilemap:
    ...
    def rect_out_of_the_map(self, pos):
        ...
        for offset in NEIGHBOR_OFFSETS:
            ...
            if check_loc not in self.tilemap:
                outside_tiles.append(
                    pygame.Rect(
                        (tile_loc[0] + offset[0]) * self.tile_size,
                        (tile_loc[1] + offset[1]) * self.tile_size,
                        self.tile_size,
                        self.tile_size,
                    )
                )
    ...
    def npc_rects(self, pos):
        ...
        if self.boss is not None:
            npc_tiles.append(
                pygame.Rect(
                    (self.boss.pos[0], self.boss.pos[1], self.boss.size, self.boss.size)
                )
            )
        return npc_tiles
    ...
    def battle_rects_around(self, pos):
        ...
        for tile in self.tiles_around(pos):
            if tile["type"] in BATTLE_TILES:
                rects.append(
                    pygame.Rect(
                        tile["pos"][0] * self.tile_size + 4,
                        tile["pos"][1] * self.tile_size + 4,
                        self.tile_size // 2,
                        self.tile_size // 2,
                    )
                )
    ...
    def forbidden_rects(self, pos):
        return (
            self.physics_rects_around(pos)
            + self.rect_out_of_the_map(pos)
            + self.npc_rects(pos)
        )

    def render(self, surf, offset=(0, 0)):
        for y in range(
            offset[1] // self.tile_size - 4,
            (offset[1] + surf.get_height()) // self.tile_size + 4,
        ):
            for x in range(
                offset[0] // self.tile_size - 4,
                (offset[0] + surf.get_width()) // self.tile_size + 4,
            ):
                loc = str(x) + ";" + str(y)
