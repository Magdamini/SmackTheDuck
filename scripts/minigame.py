...
class MiniGame(ABC):
    def __init__(self, desc, name, surf):
        ...
        self.buttons = {
            MiniGameStates.START: Button(
                self.background.right - START_BUTTON_SIZE[0] - self.button_offset,
                self.background.bottom - START_BUTTON_SIZE[1] - self.button_offset,
                pygame.transform.scale(
                    load_image("buttons/start.png"),
                    (START_BUTTON_SIZE[0], START_BUTTON_SIZE[1]),
                ),
            ),
            MiniGameStates.END: Button(
                self.background.right - EXIT_BUTTON_SIZE[0] - self.button_offset,
                self.background.bottom - EXIT_BUTTON_SIZE[1] - self.button_offset,
                pygame.transform.scale(
                    load_image("buttons/exit.png"),
                    (EXIT_BUTTON_SIZE[0], EXIT_BUTTON_SIZE[1]),
                ),
            ),
        }
    ...
    def render_header(self):
        ...
        rect.center = (
            ...
            self.background.y + self.header_height // 2,
        )
        ...
        pygame.draw.line(
            self.surf,
            (0, 0, 0),
            (self.background.x + self.border_offset - line_offset, line_y),
            (self.background.right - self.border_offset + line_offset, line_y),
        )
