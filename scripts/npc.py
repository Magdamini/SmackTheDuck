...
class NPC:
    def __init__(
        self, name, img_name, active_dialogue, dialogue, pos, active_lvls, size=16
    ):
        self.name = name
        ...
        with open(data) as data_file:
            ...
            for line in lines:
                ...
                npc = NPC(
                    ...
                    pos,
                    active_lvls,
                    map_handler.get_curr_map().tile_size,
                )
...
class DialogueWindow:
    ...
    def render_dialogue(self, surf, scale):
        ...
        surf.blit(
            name_text,
            (
                background.x + self.border_offset,
                background.y + self.header_height // 2 - name_text.get_height() // 2,
            ),
        )
        ...
        surf.blit(
            big_img,
            (
                background.right - self.border_offset - self.img_size,
                background.y + self.header_height // 2 - self.img_size // 2,
            ),
        )
        ...
        pygame.draw.line(
            surf,
            (0, 0, 0),
            (background.x + self.border_offset - line_offset, curr_y),
            (background.right - self.border_offset + line_offset, curr_y),
        )
        ...
        pygame.draw.line(
            surf,
            (0, 0, 0),
            (background.x + self.border_offset - line_offset, line_y),
            (background.right - self.border_offset + line_offset, line_y),
        )
    ...
    def render_player_text(self, surf, left, scale, curr_y):
        ...
        for i in range(len(texts)):
            txt, x, y = texts[i]
            rect = pygame.Rect(
                x, y, self.width - 2 * self.border_offset, txt.get_height()
            )
            rect.x = x
            rect.y = y
