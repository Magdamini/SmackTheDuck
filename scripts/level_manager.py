...
class LevelManager:
    ...
    def render(self, surf):
        ...
        surf.blit(
            lvl_text, ((self.width - lvl_text.get_width()) // 2, self.border_width)
        )
        ...
        xp_curr = pygame.Rect(
            xp_bar.x, xp_bar.y, int(self.xp / next_xp * self.bar_width), self.bar_height
        )
...
class NewLevelWindow:
    def __init__(self, new_level, animal):
        ...
        self.upgrade_stats = [
            (Stats.HEALTH, randint(3, 6)),
            (Stats.ATTACK, randint(2, 4)),
            (Stats.DEFENCE, randint(1, 3)),
        ]
    ...
    def render(self, surf, scale):
        ...
        surf.blit(
            choose_text, (background.centerx - choose_text.get_width() // 2, curr_y)
        )
