...
class HonkBattleScreen(BattleScreen):
    def __init__(
        self, display, game_state_manager, animal, backpack, boss, game, first_phase
    ):
        self.game = game
        self.boss = boss
        if first_phase:
            self.boss.img = pygame.transform.scale(
                load_image("honk/honk_basic.png"), (BATTLE_IMG_SIZE, BATTLE_IMG_SIZE)
            )
        else:
            self.boss.img = pygame.transform.scale(
                load_image("NPC/Basic/honk.png"), (BATTLE_IMG_SIZE, BATTLE_IMG_SIZE)
            )
        ...
        super().__init__(
            ...
            animal,
            backpack,
            background_img="scenes/honk_battle_background.png",
        )
    ...
    def handle_player_buttons(self):
        for name, button in self.player_buttons.items():
            if button.is_clicked():
                if name == "run":
                    if 50 + self.animal.battle_stats[
                        Stats.AGILITY
